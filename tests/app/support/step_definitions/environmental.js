...
When("I enter {string} in the search field", (text) => {
    cy.get(EnvLocators.searchField)
        .should('be.visible')
        .type(text);
});
    ...
    cy.get('body').then($body => {
        if ($body.find('select[name="field_complianc_value"]').length > 0) {
            ...
            cy.get('select').then($selects => {
                cy.wrap($selects.filter((i, el) => el.options && Array.from(el.options).some(opt => opt.value === statusMap[option])).first())
                    .should('be.visible')
                    .select(statusMap[option]);
            });
        }
...
Then("I verify the first result item", () => {
    cy.get('#Chitimacha-Boat-Launch > .accordion-button').contains('Chitimacha Boat Launch')
        .should('be.visible');
});

Then("I verify projects block is present", () => {
    cy.get('.block-views-blockproject-feed-blocks-block-1', { timeout: 5000 })
        .should('exist');
});

When("I click on environmental compliance link", () => {
    cy.contains('environmental compliance', { timeout: 5000 })
        .click();
    cy.get('body').should('be.visible');
});
